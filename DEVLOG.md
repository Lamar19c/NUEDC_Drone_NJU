...
## 2026-07-14 — 版本回退与信号链简化

### 移除 α-β 位置滤波器

- **动机**：α-β 滤波器使用原始位置差分计算速度预测，噪声速度导致预测偏差
- **修改**：`uwb_solver_solve` 用纯 EMA 替代 α-β（`POS_ALPHA=0.45`）
- **移除**：`POS_BETA`、`ax_vx/ay_vy/az_vz` 速度状态、`has_ab_state`

### 移除距离 EMA（后续恢复）

- 初始回退时同时移除了距离 EMA
- 测试发现定位严重衰减（仅原点附近振荡），根因为混入了 TREK1000 `mc` 解析代码
- 从 `5990b60`（准确基准版）恢复 `uwb_solver.c` + `uwb_solver.h`
- 重新应用：保留距离 EMA（`DIST_ALPHA=0.35`），仅移除 α-β

### mc 协议解析器保存

- 从 `fb0b52f` 提取 TREK1000 mc 解析代码，保存至 `uwb_parser_mc.h`
- 不参与编译，作为迁移参考

### 参数优化

- `POS_WINDOW_SIZE` 10→5：中值滤波窗口减半，滞后从 200ms 降至 100ms
- `DIST_WINDOW_SIZE` 恢复为 8，`MAX_DIST_JUMP` 恢复为 0.8m
- S3 锚点改为 `(0, 5, 1.5)`，与场地布局对齐

### 稳定基线（`268e605`）

信号链：
```
距离中值(W=8) + 跳变限幅(0.8m) → 距离EMA(α=0.35) → WLSQ 求解
  → 位置中值(W=5) → 位置EMA(α=0.45) → NMEA 输出
```
